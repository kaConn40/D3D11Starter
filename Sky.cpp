...
Sky::Sky(const wchar_t* right, const wchar_t* left, const wchar_t* up, const wchar_t* down, const wchar_t* front, const wchar_t* back, 
	 shared_ptr<Mesh>mesh,shared_ptr<SimplePixelShader> ps, shared_ptr<SimpleVertexShader> vs, Microsoft::WRL::ComPtr<ID3D11SamplerState> sampler) :
	mesh(mesh),ps(ps),vs(vs),sampler(sampler)
{
	D3D11_RASTERIZER_DESC ratDesc = {};
	{
		ratDesc.FillMode = D3D11_FILL_SOLID;
		ratDesc.CullMode = D3D11_CULL_FRONT;
		Graphics::Device->CreateRasterizerState(&ratDesc, ratState.GetAddressOf());
...
Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> Sky::GetSkyTexture() { return srv; }
...
//draws the acutal skybox
void Sky::Draw(std::shared_ptr<Camera> camera)
{
	//change render states
	Graphics::Context->RSSetState(ratState.Get());
	Graphics::Context->OMSetDepthStencilState(depthState.Get(),0);

	//set shaders from drawing
	ps->SetShader();
	vs->SetShader();

	ps->SetShaderResourceView("CubeMap", srv);
